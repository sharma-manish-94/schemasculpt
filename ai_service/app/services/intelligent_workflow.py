...
"""
        ...
        try:
            ...
            for entity_data in domain_data.get("entities", []):
                entity = DomainEntity(
                    name=self._normalize_entity_name(
                        entity_data.get("name", "Unknown")
                    ),
                    properties=entity_data.get("properties", {}),
                    relationships=entity_data.get("relationships", []),
                    operations=self._standardize_operations(
                        entity_data.get("operations", [])
                    ),
                )
                entities.append(entity)

            # Apply domain-specific intelligence
            entities = self._enhance_entities_with_domain_logic(
                entities, request.domain
            )
    ...
    def _generate_path_structure(
        self, entities: List[DomainEntity], request: GenerateSpecRequest
    ) -> List[PathOperation]:
        """
                ...
                if "update" in entity.operations:
                    operations.append(
                        PathOperation(
                            ...
                            parameters=self._generate_id_parameter(),
                            request_body=self._generate_update_request_body(
                                entity.name
                            ),
...
"""
            ...
            try:
                llm_response = await self.llm_service._call_llm_with_retry(
                    [{"role": "user", "content": schema_prompt}], None
                )
    ...
    def _validate_and_enhance_schema(
        self, llm_response: str, entity: DomainEntity
    ) -> Dict[str, Any]:
    ...
    def _generate_description(
        self, request: GenerateSpecRequest, entities: List[DomainEntity]
    ) -> str:
        """Generate API description."""
        entity_names = [entity.name for entity in entities]
        return f"API for managing {', '.join(entity_names)} in the {request.domain or 'application'} domain."

    def _enhance_entities_with_domain_logic(
        self, entities: List[DomainEntity], domain: Optional[str]
    ) -> List[DomainEntity]:
    ...
    def _fallback_domain_analysis(
        ...
        potential_entities = [
            ...
            if len(word) > 3
            and word.lower()
            not in ["create", "manage", "system", "application", "service"]
        ]
