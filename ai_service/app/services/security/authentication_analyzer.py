...
from ...schemas.security_schemas import (
    AuthenticationAnalysis, SecurityIssue, SecuritySeverity, OWASPCategory
)
...
class AuthenticationAnalyzer:
    ...
    async def analyze(self, spec: Dict[str, Any]) -> AuthenticationAnalysis:
            ...
            for scheme_name, scheme_def in security_schemes.items():
                ...
                if scheme_type == "apikey":
                    issues.append(SecurityIssue(
                        ...
                        location={"components": {"securitySchemes": {scheme_name: scheme_def}}},
                    ...
                    for flow_name, flow_def in flows.items():
                        if "scopes" not in flow_def:
                            issues.append(SecurityIssue(
                                ...
                                location={"components": {"securitySchemes": {scheme_name: {"flows": {flow_name: flow_def}}}}},
        ...
        if not has_authentication:
            recommendations.append("Implement OAuth2 or OpenID Connect for authentication")
            recommendations.append("Use HTTPS for all authentication-related communications")
        else:
            if any(s.get("type") == "apiKey" for s in security_schemes.values()):
                recommendations.append("Consider migrating from API keys to OAuth2 for better security")
            if unprotected_paths:
                recommendations.append(f"Protect {len(unprotected_paths)} unprotected endpoints")

        recommendations.append("Implement rate limiting to prevent brute force attacks")
        recommendations.append("Use strong password policies if password-based auth is used")
        recommendations.append("Implement multi-factor authentication (MFA) for sensitive operations")

        # Calculate security score
        score = self._calculate_score(has_authentication, issues, authentication_schemes)
    ...
    def _calculate_score(self, has_auth: bool, issues: List[SecurityIssue], schemes: List[str]) -> float:
