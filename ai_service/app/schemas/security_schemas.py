...
class OWASPCategory(str, Enum):
    ...
    UNRESTRICTED_SENSITIVE_DATA = "API6:2023 Unrestricted Access to Sensitive Business Flows"
...
class SecurityIssue(BaseModel):
    ...
    location: Dict[str, Any] = Field(default_factory=dict, description="Issue location in spec")
...
class AuthenticationAnalysis(BaseModel):
    ...
    authentication_schemes: List[str] = Field(default_factory=list, description="Detected auth schemes")
    issues: List[SecurityIssue] = Field(default_factory=list, description="Authentication issues")
...
class AuthorizationAnalysis(BaseModel):
    ...
    issues: List[SecurityIssue] = Field(default_factory=list, description="Authorization issues")
    recommendations: List[str] = Field(default_factory=list, description="General recommendations")
...
class DataExposureAnalysis(BaseModel):
    ...
    pii_fields_detected: List[Dict[str, Any]] = Field(default_factory=list, description="PII fields found")
    sensitive_data_fields: List[Dict[str, Any]] = Field(default_factory=list, description="Sensitive fields")
    unencrypted_sensitive_data: bool = Field(False, description="Sensitive data without encryption")
    issues: List[SecurityIssue] = Field(default_factory=list, description="Data exposure issues")
    recommendations: List[str] = Field(default_factory=list, description="General recommendations")
...
class OWASPComplianceAnalysis(BaseModel):
    ...
    compliant_categories: List[OWASPCategory] = Field(default_factory=list, description="Compliant categories")
    non_compliant_categories: List[OWASPCategory] = Field(default_factory=list, description="Non-compliant")
    issues: List[SecurityIssue] = Field(default_factory=list, description="OWASP compliance issues")
    compliance_percentage: float = Field(..., ge=0, le=100, description="Overall compliance %")
    recommendations: List[str] = Field(default_factory=list, description="General recommendations")
...
class SecurityAnalysisReport(BaseModel):
    ...
    authentication: AuthenticationAnalysis = Field(..., description="Authentication analysis")
    authorization: AuthorizationAnalysis = Field(..., description="Authorization analysis")
    data_exposure: DataExposureAnalysis = Field(..., description="Data exposure analysis")
    owasp_compliance: OWASPComplianceAnalysis = Field(..., description="OWASP compliance")
    all_issues: List[SecurityIssue] = Field(default_factory=list, description="All issues combined")
...
class SecurityAnalysisRequest(BaseModel):
    ...
    severity_threshold: SecuritySeverity = Field(SecuritySeverity.LOW, description="Minimum severity to report")
