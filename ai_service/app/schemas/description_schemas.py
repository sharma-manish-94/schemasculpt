...
class DescriptionContext(BaseModel):
    ...
    schema_type: Optional[str] = Field(
        None, alias="schemaType", description="For schemas: object, array, etc."
    )
    property_names: Optional[List[str]] = Field(
        None, alias="propertyNames", description="For schemas: property names"
    )
    operation_summary: Optional[str] = Field(
        None, alias="operationSummary", description="For operations: existing summary"
    )
    status_code: Optional[int] = Field(
        None, alias="statusCode", description="For responses: 200, 404, etc."
    )
...
class DescriptionItem(BaseModel):
    ...
    path: str = Field(
        ..., description="JSON path: /paths/~1users~1{id}/get/description"
    )
    type: str = Field(..., description="Type: operation, schema, parameter, response")
    current_description: Optional[str] = Field(
        None, alias="currentDescription", description="May be null/empty"
    )
...
class DescriptionAnalysisResponse(BaseModel):
    ...
    patches: List[JsonPatchOperation] = Field(
        ..., description="All patches to apply improvements"
    )
