...
class GraphDependency(BaseModel):
    ...
    target: str = Field(..., description="What this depends on (e.g., schema name, endpoint)")
    path: List[str] = Field(default_factory=list, description="Dependency path [A -> B -> C]")
...
class EnrichedFinding(BaseModel):
    ...
    raw_context: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Additional context, minimal"
    )
...
class EnrichedFindingSet(BaseModel):
    ...
    graph_version: str = Field(
        description="Hash of the dependency graph (for caching)"
    )

    spec_hash: str = Field(
        description="Hash of the original spec"
    )
