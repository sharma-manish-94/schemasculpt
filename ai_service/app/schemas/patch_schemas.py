...
class PatchGenerationRequest(BaseModel):
    ...
    context: dict = Field(
        default_factory=dict, description="Additional context for the fix"
    )
    suggestion_message: Optional[str] = Field(
        None, description="The suggestion message"
    )
...
class PatchGenerationResponse(BaseModel):
    ...
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Confidence level of the patch (0-1)"
    )
    warnings: List[str] = Field(
        default_factory=list, description="Any warnings about the patch"
    )
...
class PatchApplicationRequest(BaseModel):
    ...
    validate_after: bool = Field(
        default=True, description="Validate spec after applying patch"
    )
...
class PatchApplicationResponse(BaseModel):
    ...
    errors: List[str] = Field(
        default_factory=list, description="Any errors during application"
    )
    validation_errors: List[str] = Field(
        default_factory=list, description="Validation errors if validate_after=True"
    )
...
class SmartAIFixRequest(BaseModel):
    ...
    target_path: Optional[str] = Field(
        None, description="Specific API path to target (e.g., '/user/{id}')"
    )
    target_method: Optional[str] = Field(
        None, description="Specific HTTP method (e.g., 'get', 'post')"
    )
    validation_errors: List[str] = Field(
        default_factory=list, description="Specific validation errors to fix"
    )

    # Mode selection
    force_full_regeneration: bool = Field(
        default=False, description="Force full spec regeneration instead of patches"
    )
...
class SmartAIFixResponse(BaseModel):
    ...
    patches_applied: Optional[List[JsonPatchOperation]] = Field(
        None, description="Patches if patch method was used"
    )
