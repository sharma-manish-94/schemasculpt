...
class Settings(BaseSettings):
    ...
    ollama_generate_endpoint: str = Field(
        default="/api/generate", env="OLLAMA_GENERATE_ENDPOINT"
    )
    ...
    explanation_cache_ttl_hours: int = Field(
        default=24, env="EXPLANATION_CACHE_TTL_HOURS"
    )  # 24 hours
    max_concurrent_requests: int = Field(default=10, env="MAX_CONCURRENT_REQUESTS")

    # Prompt Engineering
    system_prompt_template_path: str = Field(
        default="prompts/system", env="SYSTEM_PROMPT_PATH"
    )
    user_prompt_template_path: str = Field(
        default="prompts/user", env="USER_PROMPT_PATH"
    )
