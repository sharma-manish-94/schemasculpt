...
const SecurityIssuesList = ({ issues }) => {
    ...
    const filteredAndSortedIssues = useMemo(() => {
        ...
        if (filterCategory !== 'all') {
            filtered = filtered.filter(issue => issue.owasp_category === filterCategory);
        }
                ...
                <div className="issues-filters">
                    <div className="filter-group">
                        ...
                        >
                            ...
                            <option value="critical">Critical ({issues.filter(i => i.severity?.toLowerCase() === 'critical').length})</option>
                            <option value="high">High ({issues.filter(i => i.severity?.toLowerCase() === 'high').length})</option>
                            <option value="medium">Medium ({issues.filter(i => i.severity?.toLowerCase() === 'medium').length})</option>
                            <option value="low">Low ({issues.filter(i => i.severity?.toLowerCase() === 'low').length})</option>
                            <option value="info">Info ({issues.filter(i => i.severity?.toLowerCase() === 'info').length})</option>
                        </select>
                    </div>
                            ...
                            <div className="issue-details">
                                ...
                                {issue.owasp_category && (
                                    <div className="issue-detail-section">
                                        ...
                                        <p className="owasp-tag">{issue.owasp_category.replace(/_/g, ' ')}</p>
                                    </div>
                                )}
