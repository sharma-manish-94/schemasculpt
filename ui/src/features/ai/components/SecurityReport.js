...
const SecurityReport = ({ report }) => {
    ...
    const issueCountBySeverity = (issues, severity) => {
        return issues?.filter(issue => issue.severity?.toLowerCase() === severity.toLowerCase()).length || 0;
    };
    ...
    return (
        <div className="security-report">
            ...
            <div className="security-section">
                ...
                <div className="analysis-grid">
                    <div className="analysis-stat">
                        ...
                        <span className={`stat-value ${report.authentication?.has_authentication ? 'positive' : 'negative'}`}>
                    ...
                    <div className="analysis-stat">
                        ...
                        <span className="stat-value">
                            {report.authentication?.authentication_schemes?.join(', ') || 'None'}
                        </span>
                    </div>
                    <div className="analysis-stat">
                        <span className="stat-label">Unprotected Endpoints</span>
                        <span className={`stat-value ${report.authentication?.unprotected_endpoints > 0 ? 'warning' : 'positive'}`}>
            ...
            <div className="security-section">
                ...
                <div className="analysis-grid">
                    <div className="analysis-stat">
                        ...
                        <span className={`stat-value ${report.authorization?.rbac_implemented ? 'positive' : 'negative'}`}>
                    ...
                    <div className="analysis-stat">
                        <span className="stat-label">Unprotected Endpoints</span>
                        <span className={`stat-value ${report.authorization?.unprotected_endpoints > 0 ? 'warning' : 'positive'}`}>
            ...
            <div className="security-section">
                ...
                <div className="analysis-grid">
                    <div className="analysis-stat">
                        ...
                        <span className={`stat-value ${report.data_exposure?.pii_fields_detected?.length > 0 ? 'warning' : 'positive'}`}>
                            {report.data_exposure?.pii_fields_detected?.length || 0}
                        </span>
                    </div>
                    <div className="analysis-stat">
                        <span className="stat-label">Sensitive Data Fields</span>
                        <span className={`stat-value ${report.data_exposure?.sensitive_data_fields?.length > 0 ? 'warning' : 'positive'}`}>
                            {report.data_exposure?.sensitive_data_fields?.length || 0}
                        </span>
                    </div>
                    <div className="analysis-stat">
                        <span className="stat-label">Unencrypted Sensitive Data</span>
                        <span className={`stat-value ${report.data_exposure?.unencrypted_sensitive_data ? 'negative' : 'positive'}`}>
                            {report.data_exposure?.unencrypted_sensitive_data ? '✗ Yes' : '✓ No'}
                        </span>
                    </div>
                    <div className="analysis-stat">
                        <span className="stat-label">Issues Found</span>
                ...
                {/* PII Fields Details */}
                {report.data_exposure?.pii_fields_detected && report.data_exposure.pii_fields_detected.length > 0 && (
                    <div className="pii-details">
                        <h5>Detected PII Fields</h5>
                        <div className="pii-list">
                            {report.data_exposure.pii_fields_detected.slice(0, 5).map((pii, idx) => (
                                <div key={idx} className="pii-item">
                                    <span className="pii-type">{pii.type}</span>
                                    <span className="pii-field">{pii.schema}.{pii.field}</span>
                                </div>
                            ))}
                ...
                <div className="owasp-compliance">
                    <div className="compliance-stats">
                        <div className="compliance-stat">
                            <span className="compliance-percentage">
                                {report.owasp_compliance?.compliance_percentage?.toFixed(0) || 0}%
                            </span>
                        ...
                        <div className="compliance-breakdown">
                            <div className="compliance-item">
                                ...
                                <span>{report.owasp_compliance?.compliant_categories?.length || 0} Compliant</span>
                            </div>
                            <div className="compliance-item">
                                <span className="compliance-icon negative">✗</span>
                                <span>{report.owasp_compliance?.non_compliant_categories?.length || 0} Non-Compliant</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {report.owasp_compliance?.compliant_categories?.length > 0 && (
                        <div className="owasp-category-list">
                            ...
                            <div className="category-tags">
                                {report.owasp_compliance.compliant_categories.map((cat, idx) => (
                    ...
                    {report.owasp_compliance?.non_compliant_categories?.length > 0 && (
                        <div className="owasp-category-list">
                            ...
                            <div className="category-tags">
                                {report.owasp_compliance.non_compliant_categories.map((cat, idx) => (
            ...
            <div className="security-metadata">
                <span className="metadata-item">Generated: {new Date(report.generated_at).toLocaleString()}</span>
