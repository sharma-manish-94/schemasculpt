...
function APILab() {
    ...
    const [selectedEndpoint, setSelectedEndpoint] = useState({ path: '', method: 'GET' });
        ...
        if (specText) {
            try {
                ...
                if (endpoints.length > 0 && !selectedEndpoint.path) {
                    setSelectedEndpoint({ path: endpoints[0].path, method: endpoints[0].method });
                }
            } catch (e) {
    ...
    const refreshMockServer = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/ai/mock/${mockServer.mock_id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    spec_text: specText,
            ...
            if (!response.ok) {
                throw new Error(`Failed to refresh mock server: ${response.statusText}`);
            }
    ...
    const generateMockDataVariations = async () => {
        ...
        try {
            const response = await fetch('http://localhost:8000/ai/mock/generate-variations', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    spec_text: specText,
    ...
    return (
        <div className="api-lab-container">
            ...
            <div className="api-lab-tabs">
                <button
                    className={`tab-button ${activeTab === 'mock-server' ? 'active' : ''}`}
            ...
            {activeTab === 'mock-server' && (
                <div className="tab-content">
                    ...
                    <p>Start a mock server that returns realistic, AI-generated responses</p>
                        ...
                        <div className="config-options">
                            <label className="checkbox-label">
                                <input
                                    ...
                                    onChange={(e) => setMockServerConfig({ ...mockServerConfig, useAI: e.target.checked })}
                                />
                            ...
                            <div className="config-row">
                                <label>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setMockServerConfig({ ...mockServerConfig, responseDelay: parseInt(e.target.value) || 0 })}
                                        min="0"
                                        max="5000"
                                    />
                                </label>
                            ...
                            <div className="config-row">
                                <label>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setMockServerConfig({ ...mockServerConfig, errorRate: parseFloat(e.target.value) || 0 })}
                                        min="0"
                                        max="1"
                                    />
                                </label>
                            ...
                            <div className="config-row">
                                <label>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setMockServerConfig({ ...mockServerConfig, responseVariety: parseInt(e.target.value) || 3 })}
                                        min="1"
                                        max="10"
                                    />
                                </label>
                    ...
                    <div className="action-buttons">
                        <Button onClick={startMockServer} disabled={loading || !specText}>
                            {loading ? <LoadingSpinner size="small" /> : mockServer ? 'Restart Mock Server' : 'Start Mock Server'}
                        </Button>

                        {mockServer && (
                            <Button onClick={refreshMockServer} disabled={loading} variant="secondary">
                        ...
                        <div className="mock-server-info">
                            ...
                            <div className="info-grid">
                                <div className="info-item">
                                    ...
                                    <button onClick={() => copyToClipboard(`http://${mockServer.host}:${mockServer.port}${mockServer.base_url}`)}>
                                        📋 Copy
                                    </button>
                                </div>
                                <div className="info-item">
                                    <strong>Available Endpoints:</strong> {mockServer.total_endpoints}
                                </div>
                                <div className="info-item">
                                    <strong>Status:</strong> <span className="status-badge">{mockServer.status}</span>
                                </div>
                            </div>
            ...
            {activeTab === 'test-cases' && (
                <div className="tab-content">
                    ...
                    <div className="action-buttons">
                        <Button onClick={generateTestCases} disabled={loading || !specText || !selectedEndpoint.path}>
                            {loading ? <LoadingSpinner size="small" /> : 'Generate Test Cases'}
                        </Button>
                    </div>
                    ...
                    {testCases && (
                        <div className="test-cases-results">
                            ...
                            <div className="test-summary">
                                <span className="summary-item">Total: {testCases.total_tests}</span>
                                <span className="summary-item happy">Happy Path: {testCases.happy_path_tests?.length || 0}</span>
                                <span className="summary-item sad">Sad Path: {testCases.sad_path_tests?.length || 0}</span>
                                <span className="summary-item edge">Edge Cases: {testCases.edge_case_tests?.length || 0}</span>
                                {testCases.ai_generated_tests && (
                                    <span className="summary-item ai">AI Tests: {testCases.ai_generated_tests.length}</span>
                                )}
                            </div>
            ...
            {activeTab === 'mock-data' && (
                <div className="tab-content">
                    ...
                    <div className="config-section">
                        ...
                        <div className="config-row">
                            <label>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        const [method, ...pathParts] = e.target.value.split(':');
                                        setMockDataConfig({ ...mockDataConfig, method, path: pathParts.join(':') });
                                    }}
                                    className="endpoint-selector"
                                >
                        ...
                        <div className="config-row">
                            <label>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setMockDataConfig({ ...mockDataConfig, responseCode: e.target.value })}
                                    placeholder="200"
                                />
                            </label>
                        </div>
                        ...
                        <div className="config-row">
                            <label>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setMockDataConfig({ ...mockDataConfig, variationCount: parseInt(e.target.value) || 3 })}
                                    min="1"
                                    max="10"
                                />
                            </label>
                        ...
                        <label className="checkbox-label">
                            <input
                                ...
                                onChange={(e) => setMockDataConfig({ ...mockDataConfig, useAI: e.target.checked })}
                            />
                    ...
                    <div className="action-buttons">
                        <Button onClick={generateMockDataVariations} disabled={loading || !specText || !mockDataConfig.path}>
                    ...
                    {tests.map((test, idx) => (
                        <div key={idx} className="test-case-card">
                            <div className="test-case-header">
                                <h6>{test.name}</h6>
                                <span className={`test-type-badge ${test.type}`}>{test.type}</span>
                            </div>
                            ...
                            <div className="test-details">
                                ...
                                <div className="detail-row">
                                    <strong>Expected Status:</strong> <span className="status-code">{test.expected_status}</span>
                                </div>
