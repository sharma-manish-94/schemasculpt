            ...
            switch (analyzerType) {
                case 'taint':
                    ...
                    if (rawResult.success) {
                        interpretation = await interpretTaintAnalysis(rawResult.data, specContent);
                    }
                    break;
                case 'authz':
                    rawResult = await runAuthzMatrixAnalysis(sessionId);
                    if (rawResult.success) {
                        interpretation = await interpretAuthzMatrix(rawResult.data, specContent);
                    }
                    break;
                case 'similarity':
                    rawResult = await runSchemaSimilarityAnalysis(sessionId);
                    if (rawResult.success) {
                        interpretation = await interpretSchemaSimilarity(rawResult.data, specContent);
                    }
                    break;
                case 'zombie':
                    rawResult = await runZombieApiDetection(sessionId);
                    if (rawResult.success) {
                        interpretation = await interpretZombieApis(rawResult.data, specContent);
                    }
                    break;
                default:
        ...
        <div className="advanced-analysis-tab">
            <div className="analysis-header">
                ...
                <p className="analysis-description">
                    Pre-calculated complex facts enabling instant expert-level security and governance insights
                </p>
            </div>
            ...
            <div className="analyzer-buttons">
                ...
                >
                    {analyzing && activeAnalyzer === 'comprehensive' ? '⏳' : '🎯'} Comprehensive Analysis
                </button>
                        ...
                        <div className="overview-content">
                            ...
                            {activeAnalyzer === 'comprehensive' && (
                                <div className="comprehensive-results">
                                    ...
                                    <div className="health-dashboard">
                                        <div className="main-score-card">
                                            <div
                                                ...
                                                style={{ borderColor: getHealthColor(results.interpretation.overall_health_score) }}
                                            >
                                                <span className="score-value">{results.interpretation.overall_health_score}</span>
                                                <span className="score-label">Health Score</span>
                                            </div>
                                            <div
                                                className="risk-badge-large"
                                                style={{ backgroundColor: getRiskColor(results.interpretation.risk_level) }}
                                            >
                                        ...
                                        {results.interpretation.health_breakdown && (
                                            <div className="score-breakdown">
                                                <ScoreCard
                                                    title="Security"
                                                    score={results.interpretation.health_breakdown.security_score}
                                                    icon="🔒"
                                                />
                                                <ScoreCard
                                                    title="Access Control"
                                                    score={results.interpretation.health_breakdown.access_control_score}
                                                    icon="🛡️"
                                                />
                                                <ScoreCard
                                                    title="Code Quality"
                                                    score={results.interpretation.health_breakdown.code_quality_score}
                                                    icon="✨"
                                                />
                                                <ScoreCard
                                                    title="Maintenance"
                                                    score={results.interpretation.health_breakdown.maintenance_score}
                                                    icon="🔧"
                                                />
                                            </div>
                                        )}
                                    ...
                                    {results.interpretation.top_3_critical_issues && results.interpretation.top_3_critical_issues.length > 0 && (
                                        <div className="critical-issues-section">
                                            <h4>🚨 Top 3 Critical Issues</h4>
                                            {results.interpretation.top_3_critical_issues.map((issue, idx) => (
                                                <div key={idx} className="critical-issue-card">
                                                    <div className="issue-header">
                                                        <span className="issue-category">{issue.category}</span>
                                                        <span
                                                            className="issue-urgency"
                                                            style={{ backgroundColor: getRiskColor(issue.urgency) }}
                                                        >
                                                            {issue.urgency}
                                                        </span>
                                                    </div>
                                    ...
                                    <div className="action-items-grid">
                                        {results.interpretation.immediate_action_items && results.interpretation.immediate_action_items.length > 0 && (
                                            <div className="action-section">
                                                <h4>⚡ Immediate Actions</h4>
                                                <ul>
                                                    {results.interpretation.immediate_action_items.map((item, idx) => (
                                                        <li key={idx}>{item}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                        ...
                                        {results.interpretation.short_term_roadmap && results.interpretation.short_term_roadmap.length > 0 && (
                                            <div className="action-section">
                                                <h4>📅 Short-term Roadmap (1-2 weeks)</h4>
                                                <ul>
                                                    {results.interpretation.short_term_roadmap.map((item, idx) => (
                                                        <li key={idx}>{item}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                        ...
                                        {results.interpretation.long_term_improvements && results.interpretation.long_term_improvements.length > 0 && (
                                            <div className="action-section">
                                                <h4>🎯 Long-term Improvements</h4>
                                                <ul>
                                                    {results.interpretation.long_term_improvements.map((item, idx) => (
                                                        <li key={idx}>{item}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                        ...
                                        <div className="roi-section">
                                            ...
                                            {results.interpretation.estimated_total_effort && (
                                                <p className="effort-estimate">
                                                    <strong>Estimated Effort:</strong> {results.interpretation.estimated_total_effort}
                                                </p>
                                            )}
                                        </div>
                                    )}
            ...
            {!results && !analyzing && !error && (
                <div className="analysis-empty-state">
                    ...
                    <ul className="analysis-features">
                        <li>🔍 <strong>Taint Analysis:</strong> Track sensitive data leakage paths</li>
                        <li>🛡️ <strong>Authz Matrix:</strong> Identify RBAC misconfigurations and privilege escalation risks</li>
                        <li>📦 <strong>Schema Similarity:</strong> Find duplicate schemas and refactoring opportunities</li>
                        <li>🧟 <strong>Zombie APIs:</strong> Detect unreachable endpoints and dead code</li>
                    </ul>
                </div>
            )}
        </div>
...
const SpecificAnalyzerResults = ({ analyzerType, interpretation, getRiskColor }) => {
    switch (analyzerType) {
        case 'taint':
            return <TaintAnalysisResults interpretation={interpretation} getRiskColor={getRiskColor} />;
        case 'authz':
            return <AuthzMatrixResults interpretation={interpretation} getRiskColor={getRiskColor} />;
        case 'similarity':
...
const TaintAnalysisResults = ({ interpretation, getRiskColor }) => (
    <div className="taint-results">
            ...
            <div className="issues-section">
                ...
                {interpretation.top_issues.map((issue, idx) => (
                    <div key={idx} className="issue-card">
                        ...
                        <p className="leaked-data"><strong>Leaked Data:</strong> {issue.leaked_data}</p>
                        <p className="attack-scenario"><strong>Attack Scenario:</strong> {issue.attack_scenario}</p>
                        {issue.compliance_impact && (
                            <p className="compliance-impact"><strong>Compliance:</strong> {issue.compliance_impact}</p>
                        )}
                    </div>
                ))}
            </div>
        ...
        {interpretation.remediation_priorities && interpretation.remediation_priorities.length > 0 && (
                    ...
                    <div key={idx} className="remediation-card">
                        ...
                        <div className="remediation-content">
                            ...
                            <p className="affected-endpoints">
                                Affects: {item.endpoints_affected?.join(', ') || 'Multiple endpoints'}
                            </p>
...
const AuthzMatrixResults = ({ interpretation, getRiskColor }) => (
    <div className="authz-results">
        ...
        {interpretation.anomalies_detected && interpretation.anomalies_detected.length > 0 && (
            <div className="anomalies-section">
                ...
                {interpretation.anomalies_detected.map((anomaly, idx) => (
                    <div key={idx} className="anomaly-card">
                        ...
                        <p className="endpoint"><strong>Endpoint:</strong> {anomaly.endpoint}</p>
                        <p className="issue-description">{anomaly.issue}</p>
                        <p className="current-scopes">
                            <strong>Current Scopes:</strong> {anomaly.current_scopes?.join(', ') || 'None'}
                        </p>
                        <p className="attack-scenario"><strong>Attack:</strong> {anomaly.attack_scenario}</p>
                        <p className="recommendation">
                            <strong>Recommended Scopes:</strong> {anomaly.recommended_scopes?.join(', ') || 'N/A'}
                        </p>
                    </div>
                ))}
            </div>
                ...
                {interpretation.recommendations.map((rec, idx) => (
                    <div key={idx} className="recommendation-card">
                        ...
                        {rec.affected_endpoints && rec.affected_endpoints.length > 0 && (
                            <p className="affected">Affects: {rec.affected_endpoints.join(', ')}</p>
                        )}
                    </div>
                ))}
            </div>
    ...
    <div className="similarity-results">
        ...
        <div className="summary-section">
            ...
            {interpretation.potential_savings && (
                <p className="savings"><strong>Potential Savings:</strong> {interpretation.potential_savings}</p>
            )}
        </div>

        {interpretation.refactoring_opportunities && interpretation.refactoring_opportunities.length > 0 && (
                ...
                {interpretation.refactoring_opportunities.map((opp, idx) => (
                    <div key={idx} className="refactoring-card">
                        <div className="schemas">
                            <strong>Schemas:</strong> {opp.schema_names?.join(', ') || 'N/A'}
                            <span className="similarity-score">
                                {Math.round(opp.similarity_score * 100)}% similar
                            </span>
                        </div>
                        <p className="issue">{opp.issue}</p>
                        <p className="strategy"><strong>Strategy:</strong> {opp.refactoring_strategy}</p>
                        ...
                        <div className="metadata">
                            ...
                            <span className="risk">Breaking Change Risk: {opp.breaking_change_risk}</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="quick-wins-section">
                ...
                {interpretation.quick_wins.map((win, idx) => (
                    <div key={idx} className="quick-win-card">
                        <p><strong>Schemas:</strong> {win.schemas?.join(', ')}</p>
    ...
    <div className="zombie-results">
        ...
        <div className="summary-section">
            ...
            {interpretation.maintenance_burden && (
                <p className="burden"><strong>Maintenance Burden:</strong> {interpretation.maintenance_burden}</p>
            )}
        </div>

        {interpretation.shadowed_endpoint_analysis && interpretation.shadowed_endpoint_analysis.length > 0 && (
            <div className="shadowed-section">
                ...
                {interpretation.shadowed_endpoint_analysis.map((item, idx) => (
                    <div key={idx} className="zombie-card">
                        <p><strong>Shadowed:</strong> {item.shadowed_path}</p>
                        <p><strong>By:</strong> {item.shadowing_path}</p>
                        <p className="reason">{item.reason}</p>
                        <p className="recommendation">
                            <strong>Fix:</strong> {item.recommendation} - {item.fix_instructions}
                        </p>
                        <p className="breaking">Breaking Change: {item.breaking_change}</p>
                    </div>
                ))}
            </div>
        )}

        {interpretation.orphaned_operation_analysis && interpretation.orphaned_operation_analysis.length > 0 && (
        ...
        {interpretation.cleanup_priorities && interpretation.cleanup_priorities.length > 0 && (
            <div className="cleanup-section">
                ...
                {interpretation.cleanup_priorities.map((item, idx) => (
                    <div key={idx} className="cleanup-card">
                        <div className="priority-badge">{item.priority}</div>
                        <p>{item.action}</p>
                        <p className="affected">Affects: {item.affected_endpoints?.join(', ')}</p>
                        <p className="effort">Effort: {item.estimated_effort}</p>
                    </div>
                ))}
            </div>
