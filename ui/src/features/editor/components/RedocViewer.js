...
function RedocViewer({ spec, isLoading, selectedOperation }) {
  ...
  useEffect(() => {
    if (selectedOperation && isRedocReady && redocContainerRef.current) {
      ...
      const pathSlug = selectedOperation.path.replace(/\//g, '-').replace(/[{}]/g, '');
      const operationId = `operation/${pathSlug}/${selectedOperation.method.toLowerCase()}`;

      // Attempt to scroll to operation
      setTimeout(() => {
        const operationElement = redocContainerRef.current?.querySelector(`[id="${operationId}"]`);
        if (operationElement) {
  ...
  const renderOperationBadge = () => {
    ...
    return (
      <span className={`operation-badge ${getMethodClass(selectedOperation.method)}`}>
  ...
  const getDownloadFilename = () => {
    ...
    const methodPart = selectedOperation.method ? selectedOperation.method.toLowerCase() : 'operation';
  ...
  // Main render
  return (
    <div className={`redoc-viewer-container ${viewMode}`} ref={redocContainerRef}>
