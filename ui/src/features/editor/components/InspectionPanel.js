...
function InspectionPanel() {
  ...
  const selectedNavItemDetails = useSpecStore(
    (state) => state.selectedNavItemDetails,
  );
  ...
  if (selectedNavItem) {
    ...
    if (isNavItemLoading) {
      ...
      content = (
        <LivePreview
          endpoint={selectedNavItem}
          operationDetails={selectedNavItemDetails}
        />
      );
    }
  } else {
    content = (
      <div className="panel-content-placeholder">
        {" "}
        Select an item to see a preview
      </div>
    );
  }
