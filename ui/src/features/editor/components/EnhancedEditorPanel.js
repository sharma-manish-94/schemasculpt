...
function EditorToolbar({ project }) {
  ...
  const handleDownload = () => {
    const content =
      format === "yaml" ? yaml.dump(JSON.parse(specText)) : specText;
  ...
  const handleLoadVersion = async (version) => {
    ...
    try {
      const spec = await projectAPI.getSpecificationVersion(
        project.id,
        version,
      );
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !event.target.closest('[title="Load previous version"]')
      ) {
        setShowVersions(false);
      }
    };
...
function EnhancedEditorPanel({ project }) {
  const { specText, setSpecText, format, validateCurrentSpec, sessionId } =
    useSpecStore();
