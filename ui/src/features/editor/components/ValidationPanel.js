...
const ValidationPanel = React.memo(() => {
  ...
  const groupSuggestions = (suggestions) => {
    const autoFixSuggestions = suggestions.filter(
      (sug) =>
        sug.ruleId &&
        autoFixableRules.includes(sug.ruleId) &&
        sug.category !== "ai-friendliness",
    );
    const aiFixSuggestions = suggestions.filter(
      (sug) =>
        sug.ruleId &&
        !autoFixableRules.includes(sug.ruleId) &&
        sug.category !== "ai-friendliness",
    ...
    const aiFriendlySuggestions = suggestions.filter(
      (sug) => sug.category === "ai-friendliness",
    );
  ...
  if (!hasErrors && !hasSuggestions) {
    return (
      <p className="no-errors">No validation errors or suggestions found.</p>
    );
  }
      ...
      <div className="validation-header">
        <div className="validation-counts">
          ...
          {suggestionsSummary.byCategory.Security > 0 && (
            <span
              className="count-badge security-count"
              title="Security-related suggestions"
            >
          ...
          {aiFriendlySuggestions.length > 0 && (
            <span
              className="count-badge ai-friendly-count"
              title="AI-friendliness suggestions"
            >
