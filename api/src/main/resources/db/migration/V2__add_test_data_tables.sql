...
COMMENT ON TABLE operation_test_cases IS 'Persisted test cases generated for API operations';
COMMENT ON TABLE operation_mock_data IS 'Persisted mock data variations generated for API operations';
COMMENT ON TABLE test_data_generation_history IS 'History of test data generation requests for analytics';

COMMENT ON COLUMN operation_test_cases.spec_hash IS 'SHA-256 hash of the specification to detect when regeneration is needed';
COMMENT ON COLUMN operation_test_cases.test_cases IS 'JSONB containing all generated test cases (happy path, sad path, edge cases, AI tests)';
COMMENT ON COLUMN operation_mock_data.spec_hash IS 'SHA-256 hash of the specification to detect when regeneration is needed';
COMMENT ON COLUMN operation_mock_data.mock_variations IS 'JSONB array containing generated mock data variations';
COMMENT ON COLUMN test_data_generation_history.cache_hit IS 'Whether this request was served from cache (DB or in-memory)';
