 ...
 *
 * <p>This service extracts FACTUAL, DETERMINISTIC security findings from an OpenAPI spec. Instead
 * of sending a 5MB spec to the AI, we send only these pre-processed facts.
 *
 * <p>Java does what it's best at: Finding facts with 100% accuracy. AI does what it's best at:
 * Reasoning about attack chains and patterns.
 *
 * <p>This is the professional, scalable approach to AI-powered security analysis.
 */
@Slf4j
@Service
                              ...
                              .forEach(
                                  (mediaTypeStr, mediaType) -> {
                                    if (mediaType.getSchema() != null) {
                                      ...
                                      if (schemaName != null) {
                                        List<String> fields =
                                            extractSchemaFields(schemaName, openApi);
                                        findings.add(
                                            SecurityFinding.endpointAcceptsSchema(
                                                methodStr, path, schemaName, fields));
                                        ...
                                        fields.stream()
                                            ...
                                            .forEach(
                                                field ->
                                                    findings.add(
                                                        SecurityFinding.writableSensitiveField(
                                                            methodStr, path, schemaName, field)));
                                      }
                                    }
                                  });
                        }
                                                ...
                                                if (mediaType.getSchema() != null) {
                                                  ...
                                                  if (schemaName != null) {
                                                    List<String> fields =
                                                        extractSchemaFields(schemaName, openApi);
                                                    findings.add(
                                                        SecurityFinding.endpointReturnsSchema(
                                                            methodStr, path, schemaName, fields));
                                                  }
                                                }
                                              });
    ...
    openApi
        ...
        .forEach(
            (schemaName, schema) -> {
              ...
              referencedSchemas.forEach(
                  refSchema ->
                      findings.add(
                          SecurityFinding.schemaDependency(
                              schemaName, refSchema, "Schema: " + schemaName)));
            });
