...
public class AiFriendlyResponseFormatRule implements LinterRule {
  ...
  private void lintOperation(
    ...
    if (operation.getOperationId() == null || operation.getOperationId().isBlank()) {
      suggestions.add(
          new ValidationSuggestion("Missing operationId at " + path, "AI_MISSING_OPERATION_ID"));
    }
        ...
        if (parameter.getDescription() == null || parameter.getDescription().isBlank()) {
          suggestions.add(
              new ValidationSuggestion(
                  "Parameter '"
                      + parameter.getName()
                      + "' needs a descriptive 'description' for AI"
                      + " reasoning.",
            ...
            (statusCode, response) -> {
              if (isErrorStatus(statusCode)) {
                this.validateErrorResponse(
                    path, method, statusCode, response, openApi, suggestions);
              }
            });
  }
...
""",
    ...
    Map<String, Object> metadata =
        Map.of(
            ...
            "required_fields",
            List.of("type", "title", "detail"),
            "ai_action",
            "Enables 'Error-Correction-Loop' where the agent fixes parameters based on"
                + " 'detail'.");

    return new ValidationSuggestion(
        message, "AI_UNSTRUCTURED_ERROR", "warning", "ai-operability", metadata, true);
  }
...
""";
    ...
    Map<String, Object> metadata =
        Map.of(
            "recommendation",
            "implement-standard-envelope",
            "severity_impact",
            "high_for_agents",
            "best_practices",
            List.of(
                "RFC 7807 (Problem Details for HTTP APIs)",
                "Envelope Pattern: { \"success\": boolean, \"data\": object,"
                    + " \"error\": object }"),
            "remediation_example",
            Map.of(
                "success", true,
