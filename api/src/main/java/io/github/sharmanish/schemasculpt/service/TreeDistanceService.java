...
public class TreeDistanceService {
  ...
  public double calculateSpecDistance(OpenAPI spec1, OpenAPI spec2) {
    ...
    ToDoubleBiFunction<SpecNode, SpecNode> changeCost = (n1, n2) -> {
      if (Objects.equals(n1.label(), n2.label()) &&
          Objects.equals(n1.type(), n2.type())) {
        return 0.0;
      }
      return 1.0;
    };
  ...
  private TreeData convertToTree(OpenAPI spec) {
    ...
    if (spec.getPaths() != null) {
      spec.getPaths().forEach((pathUrl, pathItem) -> {
        ...
        pathItem.readOperationsMap().forEach((method, operation) -> {
          ...
          if (operation.getParameters() != null) {
            operation.getParameters().forEach(param -> {
              ...
              SpecNode paramNode = new SpecNode(paramId, param.getName(), "Parameter");
          ...
          if (operation.getResponses() != null) {
            operation.getResponses().keySet().forEach(code -> {
    ...
    if (spec.getComponents() != null && spec.getComponents().getSchemas() != null) {
      ...
      spec.getComponents().getSchemas().forEach((name, schema) -> {
  ...
  private void traverseSchema(Graph<SpecNode, DefaultEdge> graph, SpecNode parent, String currentPath, String name, Schema<?> schema) {
    ...
    if (schema.getProperties() != null) {
      schema.getProperties().forEach((propName, propSchema) -> {
        traverseSchema(graph, schemaNode, currentPath + "." + propName, propName, propSchema);
      });
    }
