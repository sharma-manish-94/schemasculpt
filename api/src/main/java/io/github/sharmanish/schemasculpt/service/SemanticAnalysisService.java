...
public class SemanticAnalysisService {
      ...
      if (schema.getProperties() != null) {
        schema.getProperties().forEach( (propertyName, propertyValue) -> {
          ...
          if (description != null &&  !description.isEmpty()) {
            double score = this.calculateRelevance((String) propertyName, description);
            if (score < 0.1) {
              semanticReport.addIssue(schemaName, (String) propertyName, description, score);
            }
          }
        });
      }
  ...
  /**
   * Calculates similarity between a Property Name (e.g., "creationDate")
   * and its Description (e.g., "The date when the user was created").
   */
  ...
  public static class SemanticReport {
    ...
    public void addIssue(String schema, String field, String desc, double score) {
      issues.add(SemanticIssue.builder()
          ...
          .suggestion("Field name words do not appear in description. Is this description accurate")
          .build()
      );
    }
  }
