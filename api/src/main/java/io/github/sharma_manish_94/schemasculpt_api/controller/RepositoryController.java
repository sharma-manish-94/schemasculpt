...
import io.github.sharma_manish_94.schemasculpt_api.dto.repository.*;
...
public class RepositoryController {
    ...
    public Mono<ResponseEntity<RepositoryConnectionResponse>> connectRepository(
        ...
        log.info("Connection request for session: {} to provider: {}", sessionId, request.getProvider());
    ...
    public Mono<ResponseEntity<String>> disconnectRepository(
        ...
        return repositoryService.disconnect(sessionId)
                .then(Mono.just(ResponseEntity.ok("{\"success\": true, \"message\": \"Disconnected successfully\"}")))
                .onErrorResume(error -> {
