 ...
 *
 * AI agents struggle with "chatty" workflows that require many sequential calls.
 * List endpoints without pagination force agents to fetch all data at once,
 * causing performance issues and timeouts.
 */
@Component
public class PaginationSupportRule implements LinterRule {
                ...
                if (httpMethod.toString().equalsIgnoreCase("GET") &&
                    ...
                    if (!hasPagination) {
                        suggestions.add(new ValidationSuggestion(
                            String.format(
                                "AI-Friendliness: %s %s appears to return a collection but lacks pagination parameters. " +
                                "Add limit/offset or cursor-based pagination to prevent AI agents from inefficiently fetching all data.\n\n" +
                                "WHY: AI agents have limited token/memory budgets and request timeouts. Without pagination, " +
                                "agents must fetch entire collections at once, which can cause timeouts, memory exhaustion, " +
                                "or excessive costs. Pagination allows agents to retrieve data in manageable chunks and " +
                                "stop when they have enough information, dramatically improving performance and reliability.",
                                httpMethod, path
                            ),
                            RULE_ID,
                            "warning",
                            ...
                            Map.of(
                                ...
                                "example_params", List.of("limit", "offset", "page", "cursor", "next_token"),
    ...
    /**
     * Determine if an endpoint likely returns a collection based on:
     * - Path naming (plural, /items, /list)
     * - Operation summary/description
     * - Response schema (array type)
     */
    private boolean isCollectionEndpoint(String path, Operation operation) {
        ...
        boolean descIndicatesCollection = summary.contains("list") || summary.contains("all") ||
                                         description.contains("list") || description.contains("all") ||
                                         summary.contains("search") || description.contains("search");
    ...
    private boolean checkPaginationSupport(Operation operation) {
        ...
        List<String> paginationParams = List.of(
            "limit", "offset", "page", "per_page", "pagesize",
            "cursor", "next", "next_token", "continuation_token"
        );
