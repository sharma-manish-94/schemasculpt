...
public class ValidationServiceImpl implements ValidationService {
    ...
    public ValidationResult analyze(final String specText) {
        ...
        try {
            ...
            if (parseResult == null) {
                ValidationError error = new ValidationError("Failed to parse the specification - invalid format");
                return new ValidationResult(List.of(error), Collections.emptyList());
            }

            // Extract validation errors from the parser
            List<ValidationError> errors = parseResult.getMessages().stream()
            ...
            if (parseResult.getOpenAPI() != null) {
                ...
                errors = List.of(new ValidationError("Could not parse OpenAPI specification - check format and syntax"));
            }

            return new ValidationResult(errors, suggestions);

        } catch (Exception e) {
            ValidationError criticalError = new ValidationError(
                    "Failed to parse the specification. Please check for syntax errors. Details: " + e.getMessage());
    ...
    private String cleanUpValidationMessage(String message) {
        ...
        if (!cleanedMessage.isEmpty()) {
            cleanedMessage = Character.toUpperCase(cleanedMessage.charAt(0)) + cleanedMessage.substring(1);
        }
    ...
    private String serializeOpenApiToString(OpenAPI openApi) throws Exception {
        try {
            ...
            try {
                ...
                throw new Exception("Failed to serialize OpenAPI specification: " + jsonException.getMessage(), jsonException);
            }
        }
    }
}
