 ...
 *
 * AI agents benefit from consistent response structures across all endpoints.
 * This rule suggests using a standard wrapper like: {success, data, error}
 * or RFC 7807 Problem Details format for errors.
 */
@Component
    ...
    public List<ValidationSuggestion> lint(OpenAPI openApi) {
        ...
        if (!hasStandardWrapper && !hasProblemDetails) {
            suggestions.add(new ValidationSuggestion(
                ...
                "AI agents benefit from consistent response structures. " +
                ...
                "Without a standard format, agents must implement custom parsing logic for each API, " +
                "increasing complexity and error rates. Standardized wrappers enable agents to " +
                "universally detect success/failure and extract data without API-specific code.",
                RULE_ID,
                "info",
                "ai-friendliness",
                Map.of(
                    "recommendation", "standardized-response-wrapper",
                    "formats", List.of(
                        "Success wrapper: {success, data, error}",
                        "RFC 7807: {type, title, status, detail, instance}"
                    ),
                    "why", "Predictable response structures eliminate the need for custom parsing logic per API"
                ),
                true
            ));
        }
                    ...
                    operation.getResponses().forEach((statusCode, response) -> {
                        ...
                        if (statusCode.startsWith("4") || statusCode.startsWith("5")) {
                            if (response.getContent() != null &&
                                response.getContent().containsKey("application/json")) {
                                ...
                                if (mediaType.getSchema() != null &&
                                    ...
                                    if (errorSchema.getProperties() == null ||
                                        (!errorSchema.getProperties().containsKey("error") &&
                                         !errorSchema.getProperties().containsKey("type"))) {

                                        suggestions.add(new ValidationSuggestion(
                                            String.format(
                                                "Error response %s for %s %s should use a structured format " +
                                                "(e.g., RFC 7807 Problem Details) for better AI agent compatibility.\n\n" +
                                                "WHY: When errors occur, AI agents need to understand what went wrong and " +
                                                "whether the request can be retried. Structured error formats (RFC 7807) provide " +
                                                "machine-readable error codes, types, and details that agents can programmatically " +
                                                "handle, enabling automated error recovery and better user experience.",
                                            ...
                                            Map.of(
                                                ...
                                                "why", "Structured errors enable automated error recovery and retry logic"
                                            ),
                                            true
                                        ));
                                    }
