...
import io.github.sharma_manish_94.schemasculpt_api.repository.OperationMockDataRepository;
import io.github.sharma_manish_94.schemasculpt_api.repository.OperationTestCasesRepository;
import io.github.sharma_manish_94.schemasculpt_api.repository.TestDataGenerationHistoryRepository;
...
public class TestDataService {
    ...
    /**
     * Generate or retrieve cached test cases for an operation.
     * Implements two-level caching: DB (persistent) + AI service in-memory cache.
     */
    @Transactional
    public Map<String, Object> generateTestCases(
        ...
        try {
            ...
            Optional<OperationTestCases> cachedTests = testCasesRepository
                    .findByProject_IdAndPathAndMethod(project.getId(), path, method);

            if (cachedTests.isPresent() && cachedTests.get().getSpecHash().equals(specHash)) {
                log.info("Found cached test cases in DB for {}/{} (project: {})",
                        method, path, project.getId());

                // Record cache hit in history
                recordGenerationHistory(project, user, "test_cases", path, method,
                        true, null, System.currentTimeMillis() - startTime, true);

                return Map.of(
                    "test_cases", cachedTests.get().getTestCases(),
                    "total_tests", cachedTests.get().getTotalTests(),
                    "cached", true,
            ...
            log.info("Generating new test cases for {}/{} (project: {})",
                    method, path, project.getId());
            ...
            recordGenerationHistory(project, user, "test_cases", path, method,
                    true, null, System.currentTimeMillis() - startTime, wasAiCached);

            generatedTests.put("cache_source", wasAiCached ? "ai_memory" : "generated");
            return generatedTests;

        } catch (Exception e) {
            log.error("Failed to generate test cases for {}/{}: {}",
                    method, path, e.getMessage(), e);

            // Record failure in history
            recordGenerationHistory(project, user, "test_cases", path, method,
                    false, e.getMessage(), System.currentTimeMillis() - startTime, false);
    ...
    /**
     * Generate or retrieve cached mock data variations for an operation.
     * Implements two-level caching: DB (persistent) + AI service in-memory cache.
     */
    @Transactional
    public Map<String, Object> generateMockDataVariations(
        ...
        try {
            // Check database cache first
            Optional<OperationMockData> cachedMock = mockDataRepository
                    .findByProject_IdAndPathAndMethodAndResponseCode(
            ...
            if (cachedMock.isPresent() &&
                ...
                log.info("Found cached mock data in DB for {}/{} response {} (project: {})",
                        method, path, responseCode, project.getId());

                // Record cache hit in history
                recordGenerationHistory(project, user, "mock_data", path, method,
                        true, null, System.currentTimeMillis() - startTime, true);

                return Map.of(
                    "variations", cachedMock.get().getMockVariations(),
                    "count", cachedMock.get().getVariationCount(),
                    "cached", true,
                    "cache_source", "database"
                );
            }
            ...
            log.info("Generating new mock data for {}/{} response {} (project: {})",
                    method, path, responseCode, project.getId());
            ...
            log.info("Saved mock data to database for {}/{} response {}",
                    method, path, responseCode);
            ...
            log.error("Failed to generate mock data for {}/{} response {}: {}",
                    method, path, responseCode, e.getMessage(), e);
    ...
    public void invalidateCache(Long projectId, String specText) {
        ...
        log.info("Invalidating stale cache entries for project {} (new spec hash: {})",
                projectId, newSpecHash);
    ...
    /**
     * Call AI service endpoint for mock data generation.
     * This is a placeholder - should call the actual endpoint when available.
     */
    private Map<String, Object> callAIServiceForMockData(Map<String, Object> request) {
        ...
        return Map.of(
            "variations", "[]",
            "count", request.getOrDefault("count", 3),
            "cached", false
        );
    }
}
