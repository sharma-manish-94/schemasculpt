...
public class AnalysisService {
    ...
    /**
     * Builds a map where the key is a component name and the value is a set of all
     * other components/operations that depend on it.
     */
    public Map<String, Set<String>> buildReverseDependencyGraph(OpenAPI openApi) {
        ...
        if (openApi.getComponents() != null && openApi.getComponents().getSchemas() != null) {
            openApi.getComponents().getSchemas().keySet().forEach(schemaName -> reverseGraph.put(schemaName, new HashSet<>()));
        }
        ...
        if (specNode.has("components") && specNode.get("components").has("schemas")) {
            specNode.get("components").get("schemas").fields().forEachRemaining(componentEntry -> {
    ...
    private void findRefsInNode(JsonNode node, String dependentName, Map<String, Set<String>> reverseGraph) {
        if (node == null) return;
    ...
    public int calculateMaxDepth(OpenAPI openApi, Operation operation) {
        ...
        return calculateDepthRecursive(operationNode, new HashSet<>(), openApi.getComponents().getSchemas());
    }

    private int calculateDepthRecursive(JsonNode node, Set<String> visited, Map<String, Schema> allSchemas) {
        ...
        if (node.isObject()) {
            ...
            while (fields.hasNext()) {
                ...
                maxChildDepth = Math.max(maxChildDepth, calculateDepthRecursive(field.getValue(), visited, allSchemas));
            }
        } else if (node.isArray()) {
            // Iterate through array elements
            for (JsonNode element : node) {
                maxChildDepth = Math.max(maxChildDepth, calculateDepthRecursive(element, visited, allSchemas));
            }
        }
