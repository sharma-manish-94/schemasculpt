...
public class AnalysisService {
	...
	public Map<String, Set<String>> buildReverseDependencyGraph(OpenAPI openApi) {
		...
		if (openApi.getComponents() != null && openApi.getComponents().getSchemas() != null) {
			openApi.getComponents().getSchemas().keySet().forEach(schemaName -> reverseGraph.put(schemaName,
					new HashSet<>()));
		}
	...
	public Map<String, Integer> calculateAllDepths(String specText) {
		...
		Map<String, Schema> allSchemas =
				Optional.ofNullable(openApi.getComponents()).map(Components::getSchemas).orElse(Collections.emptyMap());
	...
	private Map<String, Integer> processAllOperations(OpenAPI openApi, Map<String, Schema> allSchemas) {
		Map<String, Integer> allDepths = new HashMap<>();
		Map<String, Integer> memo = new HashMap<>();
		openApi.getPaths().forEach((pathKey, pathItem) -> pathItem.readOperationsMap().forEach((method, operation) -> {
			...
			int maxDepth = calculateMaxDepth(operationNode, new HashSet<>(), allSchemas, memo);
	...
	private int calculateMaxDepth(JsonNode node, Set<String> visited, Map<String, Schema> allSchemas, Map<String,
			                                                                                                     Integer> memo) {
