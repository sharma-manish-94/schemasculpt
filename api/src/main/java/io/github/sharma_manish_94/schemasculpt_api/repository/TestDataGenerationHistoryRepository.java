...
@Repository
public interface TestDataGenerationHistoryRepository
    extends JpaRepository<TestDataGenerationHistory, Long> {

  List<TestDataGenerationHistory> findByProject_IdOrderByCreatedAtDesc(Long projectId);

  List<TestDataGenerationHistory> findByProject_IdAndDataTypeOrderByCreatedAtDesc(
      Long projectId, String dataType);

  List<TestDataGenerationHistory> findByCreatedAtAfterOrderByCreatedAtDesc(LocalDateTime after);

  @Query(
      "SELECT AVG(h.generationTimeMs) FROM TestDataGenerationHistory h WHERE h.project.id ="
          + " :projectId AND h.dataType = :dataType AND h.success = true")
  Double getAverageGenerationTime(Long projectId, String dataType);

  @Query(
      "SELECT COUNT(h) FROM TestDataGenerationHistory h WHERE h.project.id = :projectId AND"
          + " h.cacheHit = true")
