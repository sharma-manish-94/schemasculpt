...
public class HardeningService {
    ...
    public HardeningResponse hardenOperation(String sessionId, HardenOperationRequest request) {
        log.info("Hardening operation {} {} with patterns: {}",
                request.method(), request.path(), request.patterns());
        ...
        if (operation == null) {
            throw new IllegalArgumentException("Operation not found: " + request.method() + " " + request.path());
        }
        ...
        log.info("Successfully applied {} patterns to {} {}",
                appliedPatterns.size(), request.method(), request.path());
    ...
    private void applyOAuth2Security(OpenAPI openAPI, Operation operation, Map<String, Object> changes, List<String> warnings) {
    ...
    private void applyRateLimiting(Operation operation, String rateLimitPolicy, Map<String, Object> changes) {
        ...
        for (ApiResponse response : operation.getResponses().values()) {
            ...
            response.getHeaders().put("X-RateLimit-Limit",
                new Header().description("Request limit per hour").schema(new Schema<>().type("integer")));
            response.getHeaders().put("X-RateLimit-Remaining",
                new Header().description("Remaining requests").schema(new Schema<>().type("integer")));
            response.getHeaders().put("X-RateLimit-Reset",
                    ...
                    .content(new Content()
                            .addMediaType("application/json",
                                new MediaType().schema(new Schema<>()
                                    .type("object")
                                    .addProperty("error", new Schema<>().type("string").example("Rate limit exceeded"))
                                    .addProperty("retry_after", new Schema<>().type("integer").example(3600)))));
    ...
    private void applyCaching(Operation operation, String cacheTTL, Map<String, Object> changes) {
        if (!"GET".equalsIgnoreCase(operation.getTags() != null && !operation.getTags().isEmpty() ?
            operation.getTags().get(0) : "GET")) {
        ...
        if (successResponse != null) {
            ...
            successResponse.getHeaders().put("Cache-Control",
                new Header().description("Cache control directives")
                    .schema(new Schema<>().type("string").example("max-age=" + ttl)));
            successResponse.getHeaders().put("ETag",
                new Header().description("Entity tag for cache validation")
        ...
        Parameter ifNoneMatch = new HeaderParameter()
                .name("If-None-Match")
        ...
        if (!operation.getResponses().containsKey("304")) {
            operation.getResponses().put("304",
    ...
    private void applyIdempotency(Operation operation, Map<String, Object> changes) {
        ...
        String method = operation.getTags() != null && !operation.getTags().isEmpty() ?
        ...
        if (List.of("POST", "PUT", "PATCH").contains(method.toUpperCase())) {
            ...
            Parameter idempotencyKey = new HeaderParameter()
                    .name("Idempotency-Key")
                    .description("Unique key to ensure request idempotency")
                    .required(true)
                    .schema(new Schema<>().type("string").format("uuid"));

            if (operation.getParameters().stream().noneMatch(p -> "Idempotency-Key".equals(p.getName()))) {
    ...
    private void applyValidation(Operation operation, Map<String, Object> changes) {
        ...
        if (!operation.getResponses().containsKey("400")) {
            ApiResponse validationErrorResponse = new ApiResponse()
    ...
    private void applyErrorHandling(Operation operation, Map<String, Object> changes) {
        ...
        if (!operation.getResponses().containsKey("401")) {
            operation.getResponses().put("401",
                new ApiResponse().description("Unauthorized - Authentication required"));
        }

        if (!operation.getResponses().containsKey("403")) {
            operation.getResponses().put("403",
                new ApiResponse().description("Forbidden - Access denied"));
        }
            ...
            ApiResponse serverErrorResponse = new ApiResponse()
                    ...
                    .content(new Content()
                            .addMediaType("application/json",
                                new MediaType().schema(new Schema<>()
                                    .type("object")
                                    .addProperty("error", new Schema<>().type("string").example("Internal server error"))
                                    .addProperty("timestamp", new Schema<>().type("string").format("date-time"))
                                    .addProperty("request_id", new Schema<>().type("string").format("uuid")))));
